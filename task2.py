...
def get_animals(url):
    ...
    while True:
        ...
        for ul in uls:
            ...
            for name in names:
                if name.get('title')[0] in animals:  # если буква присутствует в словаре, прибавляем 1
                    animals[name.get('title')[0]] += 1
                else:  # иначе добавляем букву в словарь
                    animals[name.get('title')[0]] = 1
        links = soup.find('div', id='mw-pages').find_all('a')
        for a in links:
            if a.text == 'Следующая страница':  # проверяем есть ли ссылка на следующую страницу
